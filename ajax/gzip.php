...
$fsql = $mode == MODE_GZ ? gzopen('test.sql.gz', 'wb') : fopen('test.sql', 'wb');
...
/*
$gzfile = gzopen("test.gz", 'w9' );
print $gzfile;
$one = file_get_contents("addindex.php");
gzwrite( $gzfile, $one );
$two = file_get_contents("answer.php");
gzwrite( $gzfile, $two );
$three = file_get_contents("dropset.php");
gzwrite( $gzfile, $three );
gzclose( $gzfile );

$zd = gzopen("test.gz", "r");
$contents = gzread($zd, 10000);
file_put_contents( "ungz.txt", $contents );
gzclose($zd);
*/
/*$gzfile2 = gzopen("test.gz", 'a' );
//fseek( $gzfile, 0, SEEK_END );
print $gzfile2;
gzwrite( $gzfile2, "\r\n==========================\r\n" );
$one2 = file_get_contents("install.php");
print "<br>LEn=".strlen( $one2 );
fwrite( $gzfile2, $one2 );
$two = file_get_contents("getdb.php");
fwrite( $gzfile, $two );
$three = file_get_contents("getitem.php");
fwrite( $gzfile, $three );
